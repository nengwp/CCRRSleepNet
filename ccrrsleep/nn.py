...
def conv_1d(name, input_var, filter_shape, stride, dilations=1, padding="SAME",
            bias=None, wd=None):
...
def depthwise_conv_1d(name, input_var, filter_size, in_channels,
                      channel_multiplier=1, stride=1, dilations=1, padding="SAME",
            bias=None, wd=None):
    with tf.compat.v1.variable_scope(name) as scope:
        # Trainable parameters
        kernel = variable_with_weight_decay(
            "weights",
            shape=[filter_size, 1, in_channels, channel_multiplier],
            wd=wd
        )

        # Convolution
        output_var = tf.nn.depthwise_conv2d(
            input_var,
            kernel,
            [1, stride, 1, 1],
            padding=padding,
...
def conv_2d(name, input_var, filter_shape, stride, dilations=1, padding="SAME",
            bias=None, wd=None):
    with tf.compat.v1.variable_scope(name) as scope:
        # Trainable parameters
        kernel = variable_with_weight_decay(
...
def conv_transpose_1d(name, input_var, filter_shape, output_shape, stride, dilations=1, padding="SAME",
                      bias=None, wd=None):
    with tf.compat.v1.variable_scope(name) as scope:
        # Trainable parameters
        kernel = variable_with_weight_decay(
...
def max_pool_2d(name, input_var, pool_size, stride, padding="SAME"):
    output_var = tf.nn.max_pool2d(
        input_var,
...
def avg_pool_2d(name, input_var, pool_size, stride, padding="SAME"):
    output_var = tf.nn.avg_pool(
        input_var,
        ksize=[1, pool_size[0], pool_size[1], 1],
        strides=[1, stride[0], stride[1], 1],
...
def upsampling_1d(input_var, up_size):
    output_var = tf.compat.v1.keras.layers.UpSampling2D(
        size=(up_size, 1),
        data_format=None,
