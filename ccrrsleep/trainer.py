...
from ccrrsleep.data_loader import NonSeqDataLoader, SeqDataLoader
from ccrrsleep.model import CCRRFeatureNet, CCRRSleepNet
from ccrrsleep.optimize import adam, adam_clipping_list_lr
from ccrrsleep.utils import iterate_minibatches, iterate_batch_seq_minibatches
...
class Trainer(object):
    ...
    def __init__(
        ...
        self.interval_plot_filter = interval_plot_filter  # 间隔多久画出特征图
        self.interval_save_model = interval_save_model  # 间隔多久保存模型
        self.interval_print_cm = interval_print_cm  # 间隔多久打印混淆矩阵
    ...
    def print_performance(self, sess, output_dir, network_name,
        ...
        # Get regularization loss 获得正则化损失
...
class CCRRFeatureNetTrainer(Trainer):
    ...
    def train(self, n_epochs, resume):
        with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
            ...
            for epoch in range(sess.run(global_step), n_epochs):
                print(sess.run(learning_rate),end='')
...
class CCRRSleepNetTrainer(Trainer):
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        pretrained_model_name = "ccrrfeaturenet"

        with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
            # Build training and validation networks
            train_net = CCRRSleepNet(
                batch_size=self.batch_size,
                input_dims=self.input_dims,
                n_classes=self.n_classes,
                seq_length=self.seq_length,
