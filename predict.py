...
from ccrrsleep.data_loader import SeqDataLoader
from ccrrsleep.model import CCRRSleepNet
from ccrrsleep.nn import *
from ccrrsleep.sleep_stage import (NUM_CLASSES,
                                   EPOCH_SEC_LEN,
                                   SAMPLING_RATE)
from ccrrsleep.utils import iterate_batch_seq_minibatches
...
class CustomCCRRSleepNet(CCRRSleepNet):
    ...
    def __init__(
        ...
        super(CCRRSleepNet, self).__init__(
    ...
    def build_model(self, input_var):
        ...
        network = super(CCRRSleepNet, self).build_model(
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            def lstm_cell():
                ...
                if self.use_dropout_sequence:
                    keep_prob = 0.8 if self.is_train else 1.0
        ...
        if self.use_dropout_sequence:
            ...
            if self.is_train:
                network = tf.nn.dropout(network, keep_prob=0.8, name=name)
            else:
...
def predict(
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        ...
        for subject_idx in range(n_subjects):
            ...
            # Load testing data
            Seqdata = SeqDataLoader(data_dir, n_subjects, fold_idx)
            # SeqDataLoader = SeqDataLoader(data_dir,n_subjects,subject_idx)
            x, y =  Seqdata.load_test_data()
